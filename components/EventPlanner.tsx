    ...
    const loadPreview = async () => {
      ...
      try {
        if (file.storageType === 'url') {
          ...
          // Google Docs Preview transformation
          ...
          // Office 365 / OneDrive transformation
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(ext || '')) {
          ...
          setContent(<div className="preview-content rounded-xl overflow-auto h-full custom-scrollbar" dangerouslySetInnerHTML={{ __html: result.value }}></div>);
          ...
          setContent(<div className="preview-content rounded-xl overflow-auto h-full custom-scrollbar" dangerouslySetInnerHTML={{ __html: html }}></div>);
          ...
          setContent(
            <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-6">
              ...
              <div className="text-center">
                <p className="font-black text-white text-lg">Format Preview Unavailable</p>
                <p className="text-xs text-slate-500 max-w-xs mt-2">LibreOffice files (.odt/.ods) and other specific formats should be downloaded or linked via Google/Office 365 Cloud for full previewing.</p>
                <button onClick={() => {
                   const link = document.createElement('a');
                   link.href = URL.createObjectURL(blob!);
                   link.download = file.name;
                   link.click();
                }} className="mt-6 px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-2xl text-[10px] font-black uppercase transition-all">Download to Local Viewer</button>
              </div>
            </div>
          );
        }
      ...
      <div className="flex items-center justify-between p-5 md:px-10 md:py-6 border-b border-white/5">
        ...
        <div className="flex items-center gap-4">
          {file.storageType !== 'url' && (
             ...
             }} className="w-10 h-10 bg-white/5 hover:bg-white/10 text-white rounded-xl flex items-center justify-center transition" title="Download"><i className="fas fa-download"></i></button>
          )}
      ...
      <div className="flex-1 p-4 md:p-12 overflow-hidden flex flex-col items-center justify-center">
        {loading ? (
          <div className="flex flex-col items-center gap-5">
            ...
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Rendering Intelligence Layer...</p>
          </div>
        ) : (
...
const EventPlanner: React.FC<Props> = ({ events, contacts, directoryHandle, onAddEvent, onDeleteEvent, onUpdateEvent, onUpdateContacts }) => {
  ...
  const handleAddTask = () => {
    ...
    const newTask: ProjectTask = { id: generateId(), text: taskText, completed: false, dueDate: taskDueDate || undefined, subTasks: [] };
    onUpdateEvent({ ...selectedEvent, tasks: [...(selectedEvent.tasks || []), newTask] });
    setTaskText(''); setTaskDueDate('');
  };

  const handleAddSubTask = (parentId: string, text: string) => {
    if (!selectedEvent) return;
    const newSub: ProjectTask = { id: generateId(), text: text, completed: false, subTasks: [] };
    const updatedTasks = updateTaskInList(selectedEvent.tasks || [], parentId, (p) => ({ ...p, subTasks: [...(p.subTasks || []), newSub] }));
    onUpdateEvent({ ...selectedEvent, tasks: updatedTasks });
  };

  const handleToggleTask = (taskId: string) => {
    if (!selectedEvent) return;
    const updatedTasks = updateTaskInList(selectedEvent.tasks || [], taskId, (t) => ({ ...t, completed: !t.completed, completionDate: !t.completed ? new Date().toLocaleString() : undefined }));
    onUpdateEvent({ ...selectedEvent, tasks: updatedTasks });
  };

  const handleDeleteTask = (taskId: string) => {
    if (!selectedEvent || !confirm('Permanently remove goal?')) return;
    const filterRec = (tasks: ProjectTask[]): ProjectTask[] => tasks.filter(t => t.id !== taskId).map(t => ({ ...t, subTasks: filterRec(t.subTasks || []) }));
    onUpdateEvent({ ...selectedEvent, tasks: filterRec(selectedEvent.tasks || []) });
  };

  // Add handleAddItem to manage ledger entries
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert("Encryption/Upload protocol failure.");
    } finally {
      setUploading(false);
  ...
  const handleAddCloudLink = () => {
    if (!newLinkUrl.trim() || !selectedEvent) return;
    const newFile: ProjectFile = { id: generateId(), name: newLinkName || 'Cloud Resource', type: 'url', size: 0, timestamp: new Date().toLocaleString(), storageRef: newLinkUrl, storageType: 'url' };
    onUpdateEvent({ ...selectedEvent, files: [...(selectedEvent.files || []), newFile] });
    ...
    <div className="space-y-6 pb-20 max-w-6xl mx-auto px-2">
      <div className="flex justify-between items-end mb-8">
        ...
        {!selectedEventId && (
          <button onClick={() => setShowAddForm(!showAddForm)} className={`flex items-center gap-3 px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${showAddForm ? 'bg-rose-50 text-rose-600' : 'bg-slate-900 text-white shadow-2xl'}`}>
            ...
            {showAddForm ? 'Abort' : 'Initiate Frame'}
          </button>
        )}
      </div>
        ...
        <div className="p-12 bg-white border border-slate-200 rounded-[4rem] shadow-sm animate-in zoom-in-95 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="md:col-span-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-3 block">Project Designation</label>
              <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g. Q4 Strategic Review" className="w-full p-6 bg-slate-50 border border-slate-100 rounded-3xl outline-none font-black text-lg focus:ring-4 focus:ring-indigo-50 transition-all" />
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-3 block">Initial Deadline</label>
              <input type="date" value={newDate} onChange={(e) => setNewDate(e.target.value)} className="w-full p-6 bg-slate-50 border border-slate-100 rounded-3xl outline-none font-black focus:ring-4 focus:ring-indigo-50 transition-all" />
            </div>
          </div>
          <button onClick={() => { if (!newName) return; onAddEvent({ name: newName, date: newDate, status: 'active' }); setShowAddForm(false); setNewName(''); }} className="w-full mt-12 py-6 bg-indigo-600 text-white font-black rounded-[2.5rem] shadow-2xl uppercase tracking-[0.2em] text-[12px] hover:bg-slate-900 transition-all">Establish Environment</button>
        </div>
      )}
      ...
      {!selectedEventId ? (
          ...
          {(events || []).map(event => {
            ...
            return (
              <div key={event.id} onClick={() => { setSelectedEventId(event.id); setActiveTab('ledger'); }} className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm cursor-pointer hover:shadow-2xl hover:-translate-y-2 transition-all group relative overflow-hidden flex flex-col h-full min-h-[300px]">
                <div className="absolute top-0 right-0 p-8 opacity-[0.03] scale-[2.5] rotate-12 group-hover:rotate-0 transition-transform"><i className="fas fa-microchip text-slate-900"></i></div>
                <div className="relative z-10 flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100"><p className="text-[9px] text-slate-400 font-black uppercase mb-1">Vault</p><p className="text-base font-black text-slate-800">{(event.files || []).length} Assets</p></div>
                    <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100"><p className="text-[9px] text-slate-400 font-black uppercase mb-1">Goals</p><p className="text-base font-black text-slate-800">{(event.tasks || []).length} Lines</p></div>
                  </div>
                </div>
                <div className="space-y-3 relative z-10">
                  <div className="flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest"><span>System Progress</span><span>{taskProgress.toFixed(0)}%</span></div>
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden shadow-inner"><div className="h-full bg-indigo-600 shadow-lg shadow-indigo-200 transition-all duration-1000" style={{ width: `${taskProgress}%` }}></div></div>
                </div>
              </div>
            );
          })}
          {(!events || events.length === 0) && <div className="col-span-full py-32 text-center border-2 border-dashed border-slate-200 rounded-[4rem] bg-white/50"><i className="fas fa-folder-open text-slate-200 text-6xl mb-6"></i><p className="text-slate-400 text-sm font-black uppercase tracking-[0.3em]">No project containers identified</p></div>}
        </div>
      ) : selectedEvent && (
        <div className="animate-in fade-in slide-in-from-bottom-6 duration-500">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10 bg-slate-900 p-10 rounded-[4rem] shadow-2xl relative overflow-hidden">
             <div className="absolute top-0 right-0 p-12 opacity-5 scale-[2] pointer-events-none"><i className="fas fa-fingerprint text-[140px] text-white"></i></div>
             <div className="relative z-10 flex items-center gap-6">
               ...
               <div>
                 ...
                 <div className="flex items-center gap-3 mt-2">
                   ...
                   <p className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.3em]">{selectedEvent.status} Container</p>
                 </div>
               </div>
             </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-10 animate-in fade-in">
                  ...
                  <div className="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm">
                    ...
                    <div className="space-y-6">
                      {(selectedEvent.tasks || []).map(task => (
                        <TaskItem key={task.id} task={task} depth={0} onToggle={handleToggleTask} onDelete={handleDeleteTask} onEdit={setEditingTask} onAddSub={handleAddSubTask} />
                      ))}
                ...
                <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm h-fit sticky top-28">
                  ...
                  <div className="space-y-6">
                    ...
                    <button onClick={handleAddTask} className="w-full py-5 bg-indigo-600 text-white font-black rounded-3xl shadow-2xl uppercase tracking-widest text-[10px] hover:bg-slate-900 transition-all active:scale-95">Assign Goal</button>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="animate-in fade-in space-y-10">
                <div className="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm min-h-[500px]">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                    <div>
                      <h3 className="font-black text-slate-800 uppercase text-[11px] tracking-[0.4em]">Vault: Advanced Explorer</h3>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 flex items-center gap-2">
                        <i className={`fas ${directoryHandle ? 'fa-hdd text-emerald-500' : 'fa-database text-indigo-500'}`}></i>
                        Source: {directoryHandle ? `/${selectedEvent.name.toLowerCase().replace(/\s/g, '_')}` : 'Encrypted IndexedDB'}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                       <button onClick={() => setShowAddLink(!showAddLink)} className={`px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm ${showAddLink ? 'bg-rose-50 text-rose-600' : 'bg-indigo-50 text-indigo-600'}`}><i className="fas fa-link mr-2"></i> {showAddLink ? 'Close' : 'Cloud Asset'}</button>
                       <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-2xl disabled:opacity-50">
                  ...
                  {showAddLink && (
                    <div className="mb-12 p-10 bg-slate-50 rounded-[3rem] border border-slate-200 animate-in slide-in-from-top-4 duration-300">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div><label className="text-[9px] font-black text-slate-400 uppercase ml-2 mb-2 block">Resource Label</label><input value={newLinkName} onChange={e => setNewLinkName(e.target.value)} placeholder="e.g. Office 365 Budget Sheet" className="w-full p-5 bg-white border border-slate-200 rounded-2xl outline-none font-black text-xs focus:ring-4 focus:ring-indigo-50" /></div>
                        <div><label className="text-[9px] font-black text-slate-400 uppercase ml-2 mb-2 block">Direct Access URL</label><input value={newLinkUrl} onChange={e => setNewLinkUrl(e.target.value)} placeholder="https://docs.google.com/..." className="w-full p-5 bg-white border border-slate-200 rounded-2xl outline-none font-black text-xs focus:ring-4 focus:ring-indigo-50" /></div>
                      </div>
                      <div className="flex justify-end gap-3 mt-8">
                        <button onClick={() => setShowAddLink(false)} className="px-6 py-2 text-slate-400 text-[10px] font-black uppercase hover:text-slate-600 transition">Cancel</button>
                        <button onClick={handleAddCloudLink} className="px-8 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-xl shadow-indigo-100 active:scale-95 transition-all">Link Resource</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10">
                    {(selectedEvent.files || []).map(file => {
                      ...
                      const isPDF = ext === 'pdf';
                      const isDoc = ['docx','odt'].includes(ext||'');
                        ...
                        <div key={file.id} className="group relative bg-slate-50 border border-slate-100 rounded-[3rem] p-8 text-center hover:bg-white hover:shadow-2xl transition-all border-b-4 border-b-transparent hover:border-b-indigo-500">
                          <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center text-3xl mx-auto mb-6 transition-all group-hover:scale-110 shadow-sm ${file.storageType === 'url' ? 'bg-indigo-50 text-indigo-500' : 'bg-white text-slate-400'}`}>
                            {file.storageType === 'url' ? <i className="fas fa-cloud"></i> : 
                             ...
                             isPDF ? <i className="fas fa-file-pdf text-rose-500"></i> :
                             isDoc ? <i className="fas fa-file-word text-blue-500"></i> :
                             isSheet ? <i className="fas fa-file-excel text-emerald-600"></i> :
                          ...
                          <div className="mt-8 flex gap-2">
                             <button onClick={() => setPreviewFile(file)} className="flex-1 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-100 active:scale-95">Preview</button>
                          </div>

                          <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all">
                            <button onClick={(e) => { e.stopPropagation(); if(confirm('Purge asset from vault?')) onUpdateEvent({ ...selectedEvent!, files: (selectedEvent!.files || []).filter(f => f.id !== file.id) }); }} className="w-9 h-9 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center hover:bg-rose-500 hover:text-white transition shadow-sm"><i className="fas fa-times text-[11px]"></i></button>
                          </div>
                          {file.storageType === 'filesystem' && (
                            <div className="absolute top-4 left-4 w-9 h-9 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center text-[12px] shadow-sm" title="Physical Storage Active">
                    ...
                    {(!selectedEvent.files || selectedEvent.files.length === 0) && <div className="col-span-full py-32 text-center border-2 border-dashed border-slate-100 rounded-[3rem]"><p className="text-[11px] font-black text-slate-300 uppercase tracking-[0.4em]">Repository currently void of assets</p></div>}
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="lg:col-span-3 space-y-10">
                  <div className="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm">
                    <div className="flex justify-between items-center mb-12">
                      <div><h3 className="font-black text-slate-800 uppercase text-[11px] tracking-[0.4em]">Operational Flow</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Real-time P&L Ledger</p></div>
                      <div className="text-right"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Project Net</p><p className={`text-3xl font-black ${stats.net >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>${stats.net.toLocaleString()}</p></div>
                    </div>
                      ...
                      {(selectedEvent.items || []).map(item => (
                        <div key={item.id} className="flex items-center justify-between p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 transition-all hover:bg-white hover:shadow-xl group">
                          <div className="flex items-center gap-6">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white text-xl ${item.type === 'income' ? 'bg-emerald-500' : 'bg-rose-500'} shadow-lg`}>
                              <i className={`fas ${item.type === 'income' ? 'fa-arrow-up' : 'fa-arrow-down'}`}></i>
                            </div>
                            <div>
                              <p className="font-black text-base text-slate-800 leading-none mb-2">{item.description}</p>
                              <div className="flex items-center gap-3">
                          ...
                          <p className={`font-black text-lg ${item.type === 'income' ? 'text-emerald-600' : 'text-rose-600'}`}>{item.type === 'income' ? '+' : '-'}${item.amount.toLocaleString()}</p>
                        </div>
                      ))}
                      {(!selectedEvent.items || selectedEvent.items.length === 0) && <div className="py-32 text-center border-2 border-dashed border-slate-100 rounded-[3rem] text-[11px] font-black text-slate-300 uppercase tracking-[0.2em]">Ledger sequence empty</div>}
                    </div>
                  </div>
                </div>
                <div className="space-y-10">
                  <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm">
                    <h3 className="font-black text-slate-800 uppercase text-[11px] tracking-[0.3em] mb-10">Authorize Movement</h3>
                    <form onSubmit={handleAddItem} className="space-y-6">
                      <select name="type" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-[11px] uppercase outline-none focus:ring-4 focus:ring-indigo-50"><option value="expense">Expense (-)</option><option value="income">Revenue (+)</option></select>
                      <select name="category" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-[11px] uppercase outline-none focus:ring-4 focus:ring-indigo-50">{EVENT_ITEM_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select>
                      <input name="description" required placeholder="Description..." className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-xs outline-none focus:ring-4 focus:ring-indigo-50" />
                      <input name="amount" type="number" step="0.01" required placeholder="Amount ($)..." className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-sm outline-none focus:ring-4 focus:ring-indigo-50" />
      ...
      {editingTask && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-lg rounded-[4rem] p-12 shadow-2xl border border-white/10 animate-in zoom-in-95">
            <h3 className="text-2xl font-black text-slate-800 mb-10 tracking-tight">Adjust Objective</h3>
            <form onSubmit={(e) => { e.preventDefault(); onUpdateEvent({ ...selectedEvent!, tasks: updateTaskInList(selectedEvent!.tasks || [], editingTask.id, () => editingTask) }); setEditingTask(null); }} className="space-y-8">
              ...
              <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-2 block">Critical Deadline</label><input type="date" value={editingTask.dueDate || ''} onChange={e => setEditingTask({...editingTask, dueDate: e.target.value})} className="w-full p-6 bg-slate-50 border border-slate-100 rounded-3xl outline-none font-black focus:ring-4 focus:ring-indigo-50 transition-all" /></div>
              <div className="flex gap-4 pt-4">
                <button type="submit" className="flex-1 py-6 bg-indigo-600 text-white font-black rounded-[2rem] shadow-2xl shadow-indigo-200 uppercase tracking-widest text-[12px] hover:bg-slate-900 transition-all active:scale-95">Update Frame</button>
                <button type="button" onClick={() => setEditingTask(null)} className="px-10 py-6 bg-slate-100 text-slate-400 font-black rounded-[2rem] uppercase tracking-widest text-[12px] hover:bg-slate-200 transition-all">Cancel</button>
              </div>
            </form>
          </div>
        </div>
