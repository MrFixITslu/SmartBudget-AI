...
const Settings: React.FC<Props> = ({ 
      ...
      if ("Notification" in window) {
        Notification.requestPermission().then(permission => {
          if (permission === "granted") {
            ...
            alert("Notification permission denied. Please enable them in your browser settings to use reminders.");
            onToggleReminders(false);
          }
        });
      } else {
  ...
  const budgetableCategories = CATEGORIES.filter(c => !['Income', 'Savings', 'Investments', 'Other'].includes(c));
          ...
          <section className="bg-indigo-50/50 p-6 rounded-[2rem] border border-indigo-100">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-800">Daily Desktop Reminders</h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Notification Alert for Data Entry</p>
                </div>
              </div>
              <button 
                onClick={() => handleReminderToggle(!remindersEnabled)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${remindersEnabled ? 'bg-indigo-600' : 'bg-slate-300'}`}
              >
          ...
          <section className="p-8 bg-slate-900 text-white rounded-[2.5rem] shadow-xl">
            ...
            <div className="space-y-8">
               <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4">Final Savings Objective After All Costs</p>
               ...
               <div className="pt-4 border-t border-white/10">
                  <h3 className="text-lg font-black mb-1">Monthly Category Limits</h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-6">Budget your Net Margin (Groceries, Fun, etc.)</p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {budgetableCategories.map(cat => (
                      <div key={cat} className="bg-white/5 border border-white/5 rounded-2xl p-4 flex items-center justify-between group hover:bg-white/10 transition-colors">
                        <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{cat}</p>
                          <div className="relative">
                            <span className="absolute left-0 top-1/2 -translate-y-1/2 font-black text-indigo-400 text-xs">$</span>
                            <input 
                              type="number"
                              value={categoryBudgets[cat] || ''}
                              onChange={(e) => handleBudgetChange(cat, e.target.value)}
                              placeholder="0.00"
                              className="bg-transparent border-none p-0 pl-3 outline-none font-black text-sm text-white w-24 placeholder:text-white/10"
                            />
                          </div>
                        </div>
